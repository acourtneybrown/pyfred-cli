...
def external_script(fn: Callable[[Path, list[str], Optional[Environment]], Union[str, list[str]]]):
    """
    Decorator for an external script

    Preprocesses the input and parses environment variables. The main function of an external script should be decorated
    with this.
    """
    path, args = (sys.argv[0], sys.argv[1:])
    alfred_environment = Environment.from_env()
    ...
    def decorator():
        ...
        if not isinstance(output, str) and not isinstance(output, list):
            logging.error(
                "The workflow returned an unexpected type: %s, but expected %s.%s.",
                type(output),
                ScriptFilterOutput.__module__,
        ...
        print(*output if isinstance(output, list) else output)
